...
// Variables para dibujado de Ruta
var lines;
var drawLine;
var modifyLine;

// Variables para Ubicacion de Vehiculos en Lugares
var linesVehiclesInPlace;
var drawLineVehiclesInPlace;
...
function loadMap() {
    if (connectionMap()) {
        Ext.onReady(function () {
            ...
            linesVehiclesInPlace = new OpenLayers.Layer.Vector("Geocerca Vehiculos en Lugares", {
                styleMap: new OpenLayers.StyleMap({
                    pointRadius: 3,
                    strokeColor: "#046CD5",
                    strokeWidth: 3,
...
//                    positionPoint = false;
            ...
            var styleTravel = new OpenLayers.StyleMap({
                ...
                idTravel: "${idTravel}",
                label: "${idTravel}",
                isSkpTravel: "${isSkpTravel}",
                idVehicleTravel: "${idVehicleTravel}",
                idTypeEventTravel: "${idTypeEventTravel}",
            ...
            lienzoPoinTravel = new OpenLayers.Layer.Vector('Puntos de Recorrido', {
                eventListeners: {
                    featureselected: function (evt) {
                        ...
                        var idVehicle = parseInt(feature.attributes.idVehicleTravel);
                        var typeEventTravel;
                        if (parseInt(feature.attributes.isSkpTravel) === 1) {
                            typeEventTravel = formatSkyEvent(parseInt(feature.attributes.idTypeEventTravel))
                        } else {
                            typeEventTravel = formatHeader(parseInt(feature.attributes.idTypeEventTravel))
                        }

                        var idTravel = feature.attributes.idTravel;
                        var companyTravel = formatCompany(storeVehicle.getById(idVehicle).get('idCompany'));
                        var muniRegTravel = storeVehicle.getById(idVehicle).get('muniReg');
                        var dateTimeTravel = feature.attributes.dateTimeTravel;
                        var speedTravel = feature.attributes.speedTravel;

                        var contenidoAlternativo =
                                "<section>" +
                                "<b>Trama: </b>" + idTravel.toString() + "<br>" +
                                "<b>Empresa: </b>" + companyTravel + "<br>" +
                                "<b>Registro Municipal: </b>" + muniRegTravel + "<br>" +
                                "<b>Fecha y Hora: </b>" + dateTimeTravel.toString() + "<br>" +
                                "<b>Velocidad: </b>" + speedTravel.toString() + " Km/h<br>" +
                                "<b>Evento: </b>" + typeEventTravel + "<br>" +
                                "</section>";
            ...
            drawLineVehiclesInPlace = new OpenLayers.Control.DrawFeature(linesVehiclesInPlace, OpenLayers.Handler.Polygon, {featureAdded: getDataVehiclesInPlace});
            ...
            map.addControl(drawLineVehiclesInPlace);
    ...
    if (lienzoP === null) {
        ...
        if (lienzoP.getVisibility()) {
                ...
                if (cmp === undefined) {
                    ...
                    if (!cmp.checked) {
                        ...
                        Ext.Msg.show({
                            ...
                            wait: true,
                            waitConfig: Ext.create('Ext.ProgressBar').wait({
                                interval: 1000,
                                duration: 7000,
                                increment: 7,
                                fn: function () {
                        ...
                        Ext.Msg.updateProgress(0, 'Cargando...');
                    } else {
            ...
            Ext.MessageBox.show({
                ...
                msg: 'Debe activar primero la capa de la Empresa.',
                buttons: Ext.MessageBox.OK,
                icon: Ext.MessageBox.ERROR
            });
            return null;
...
function localizarDireccion(ln, lt, zoom) {
    if (ln === 0 || lt === 0) {
        Ext.example.msg('Mensaje', 'Coordenadas Incorrectas');
        return false;
    } else {
        ...
        punto.transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject());
        centrarMapa(punto.x, punto.y, zoom);
...
function drawLineAdminRoute(dataRoute, invert) {
    lines.destroyFeatures();
    var puntosRuta = new Array();

    var json = dataRoute.verticesRoute.split(";");
    Ext.getCmp('numberfield-point-route').setValue(json.length);
    if (invert) {
        for (var i = json.length - 1; i >= 0; i--) {
            var dataRuta = json[i].split(",");

            var pt = new OpenLayers.Geometry.Point(dataRuta[1], dataRuta[0]);
    ...
    var style = {
        strokeColor: dataRoute.colorRoute,
        strokeOpacity: 1,
        strokeWidth: 4
    };
    ...
    formAdminRoute.down('[name=verticesRoute]').setValue(auxPointsRoute.substring(0, auxPointsRoute.length - 1));
    drawRoute = false;
}

function drawLineAdminRouteByPoint(json, invert) {
    lines.destroyFeatures();
    var puntosRuta = new Array();
...
function drawLineRouteManual(json) {
    ...
    for (var i = 0; i < json.length - 3; i++) {
        ...
        if (dataRecRuta.longitud !== 0 && dataRecRuta.latitud !== 0) {
...
function drawPointsTravel(records, isSkp) {
    ...
    for (var i = 0; i < records.length; i++) {
        ...
        if (isSkp) {
            var pt = new OpenLayers.Geometry.Point(data.longitudData, data.latitudData);
            pt.transform(new OpenLayers.Projection("EPSG:4326"),
                    new OpenLayers.Projection("EPSG:900913"));

            var puntoMap = new OpenLayers.Feature.Vector(pt, {
                idTravel: data.idData,
                isSkpTravel: 1,
                idVehicleTravel: data.idVehicleData,
                idTypeEventTravel: data.idSkyEventData,
                dateTimeTravel: Ext.Date.format(new Date(data.dateTimeData), 'Y-m-d H:i:s'),
                speedTravel: data.speedData,
                colorTravel: storeSkyEvent.getById(data.idSkyEventData).get('color'),
                poppedup: false
            });
            ...
            if (data.idEnc !== "X") {
                ...
                var puntoMap = new OpenLayers.Feature.Vector(pt, {
                    ...
                    isSkpTravel: 2,
                    idVehicleTravel: data.idVehicleData,
                    idTypeEventTravel: data.idHeaderData,
                    dateTimeTravel: Ext.Date.format(new Date(data.dateTimeData), 'Y-m-d H:i:s'),
                    speedTravel: data.speedData,
                    colorTravel: storeHeader.getById(data.idHeaderData).get('color'),
                    poppedup: false
                });
...
function getDataRoute(fig) {
    ...
    Ext.getCmp('btn-distance').enable();
    drawLine.deactivate();
    winAdminRoute.show();
...
function getDataVehiclesInPlace(fig) {
    var listPoint = fig.geometry.getVertices();
    var auxPointsRoute = getDataOfFeature(listPoint);
    var form = Ext.getCmp('form-vehicle-in-place');
    form.down('[name=verticesPlace]').setValue(auxPointsRoute.substring(0, auxPointsRoute.length - 1));
    thisDrawPlace = false;
    drawLineVehiclesInPlace.deactivate();
    linesVehiclesInPlace.destroyFeatures();
    winVehiclesInPlace.show();

    form.getForm().submit({
        url: 'php/gui/tools/getVehiclesInPlace.php',
        waitTitle: 'Realizando Busqueda',
        waitMsg: 'Buscando...',
        failure: function (form, action) {
            Ext.MessageBox.show({
                title: 'Mensaje',
        ...
        success: function (form, action) {
            var resultado = action.result;
            gridVehiclesInPlace.getStore().loadData(resultado.data);
        }
    });
}
