...
if (!$mysqli = getConectionDb()) {
    ...
    $consultaSql = "select u.usuario, c.comando, c.respuesta, "
            . "c.fecha_hora_registro, c.fecha_hora_envio, c.fecha_hora_respuesta,tec.tipo_estado_cmd "
            . "from kbushistoricodb.comandos c, usuarios u, tipo_estado_cmds tec "
            . "where c.id_usuario = u.id_usuario "
            . "and c.id_tipo_estado_cmd = tec.id_tipo_estado_cmd "
            . "and c.id_equipo = ? "
            . "and c.fecha_hora_registro between concat(?,' ', ?) and concat(?,' ', ?) "
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "{failure: true, message:'No hay datos entre estas Fechas '}";
        }
    } else {
